...
*/
MsgCatcherWiget.prototype.invokeMsgActions = function({event:event,continue:false}) {
	for(var t=0; t<this.children.length; t++) {
		var child = this.children[t];
		var params = {event:event,continue:false};
		if(child.invokeMsgAction) params = child.invokeMsgAction(params)) 
	}
	if(param.continue && this.parentWidget) {
		this.parentWidget.dispatchEvent(param.event);
	}
};
